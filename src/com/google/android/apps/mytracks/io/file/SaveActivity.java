...
public class SaveActivity extends Activity {
   ...
   */
  public void onAsyncTaskCompleted(boolean isSuccess, int aMessageId, final String aSavedPath) {
    ...
    // upload csv file format to our server
    if (this.success && trackFileFormat == TrackFileFormat.CSV) {
      Log.d(TAG, aSavedPath);
      new Thread(new Runnable() {
        public void run() {
          //GPS data upload
          ServerConnInterface.doFileUpload(aSavedPath,"http://209.129.244.24/louis/getUploadFile.php");
          //accelerometer data upload
          File file = new File(getExternalCacheDir(),"tracks"+trackId+".csv");
          if(file.exists()){
            ServerConnInterface.doFileUpload(file.getAbsolutePath(),"http://209.129.244.24/louis/getUploadFile.php");
          }
          else{
